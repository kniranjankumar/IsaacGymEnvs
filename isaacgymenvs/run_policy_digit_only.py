...
def apply_forces_to_rigid_bodies(_rigid_body_pos,_rigid_body_rot, _rigid_body_vel,_rigid_body_ang_vel):
        ...
        print("printing forces",forces, force_positions)
        gym.apply_rigid_body_force_at_pos_tensors(sim, gymtorch.unwrap_tensor(forces), 
...
trained_weights = torch.load("/home/dan/Projects/dynamic_motion_imitation/IsaacGymEnvs/isaacgymenvs/runs/Digit_hop_23-18-51-41/nn/Digit_hop_23-18-52-00.pth")
# trained_weights = torch.load("/home/dan/Projects/dynamic_motion_imitation/IsaacGymEnvs/issacgymenvs/runs/Digit_hop_23-18-51-41/nn/Digit_hop_23-18-52-00.pth")
...
# asset_options.use_articulations = False
...
right_foot_idx = gym.find_asset_rigid_body_index(digit_asset, "right_toe_roll")
left_foot_idx = gym.find_asset_rigid_body_index(digit_asset, "left_toe_roll")
sensor_pose = gymapi.Transform()

gym.create_asset_force_sensor(digit_asset, right_foot_idx, sensor_pose)
gym.create_asset_force_sensor(digit_asset, left_foot_idx, sensor_pose)
...
actuator_props = gym.get_asset_dof_properties(digit_asset)
for i in range(digit_num_dofs):
    actuator_props['driveMode'][i] = gymapi.DOF_MODE_POS
    actuator_props['stiffness'][i] = 500 #self.Kp
    actuator_props['damping'][i] = 0.1 #self.Kd
...
# humanoid_pose.p = gymapi.Vec3(0.0, 2, 1.0)
# humanoid_pose.r = gymapi.Quat(0, 0.0, 0.0, 1)
digit_pose = gymapi.Transform()
digit_pose.p = gymapi.Vec3(*get_axis_params(1.0, 2))
digit_pose.r = gymapi.Quat(0.0, 0.0, 0.0,1)

# humanoid_actor_handle = gym.create_actor(env, humanoid_asset, humanoid_pose, "humanoid_actor", 0, 1,0)
digit_actor_handle = gym.create_actor(env, digit_asset, digit_pose, "digit_actor", 0, 1,0)
...
for i in range(digit_num_dofs):
    actuator_props['driveMode'][i] = gymapi.DOF_MODE_POS
    actuator_props['stiffness'][i] = 200 #self.Kp
    actuator_props['damping'][i] = 10.0 #self.Kd
    actuator_props["armature"][i] = 0.0001
    actuator_props["effort"][i] = 500
    if i in toe_joints:
        actuator_props["effort"][i] = 5000
...
gym.set_actor_dof_properties(env, digit_actor_handle, actuator_props)
gym.enable_actor_dof_force_sensors(env, digit_actor_handle)
...
rigid_body_pos = gymtorch.wrap_tensor(rigid_body_pos_).view(digit_num_bodies,13)
...
_initial_root_states = root_states.clone()
_initial_root_states[:, 7:13] = 0
_initial_root_states[:,3:6] = 0
_initial_root_states[:,2]=0.93
...
digit_dof_states = dof_states.view(digit_num_dofs,2)[:digit_num_dofs,:]
digit_dof_pos = dof_states.view(digit_num_dofs,2)[:digit_num_dofs,0]
digit_dof_vel = dof_states.view(digit_num_dofs,2)[:digit_num_dofs,1]
digit_rigid_body_states = rigid_body_pos.view(digit_num_bodies,13)[:,:]
...
digit_root_states = root_states.view(1,13)[0,:]
...
dof_pos = torch.zeros((digit_num_dofs,1),device=device)
...
_initial_dof_pos=torch.tensor([ 2.7886e-01, -1.5718e-03,  2.4364e-01, -1.6560e-01,  1.8958e-01,
    ...
    2.6128e-06, -3.3921e-04]).to(device)
...
_initial_dof_vel = torch.zeros_like(digit_dof_vel, device=device, dtype=torch.float)
...
digit_dof_vel[:] = _initial_dof_vel
env_ids_int32 = torch.tensor([0], device=device, dtype=torch.int32)
gym.set_actor_root_state_tensor_indexed(sim, gymtorch.unwrap_tensor(_initial_root_states),
                                                     gymtorch.unwrap_tensor(env_ids_int32), len(env_ids_int32))

gym.set_dof_state_tensor_indexed(sim, gymtorch.unwrap_tensor(digit_dof_states), 
                                 gymtorch.unwrap_tensor(env_ids_int32), len(env_ids_int32))
gym.simulate(sim)
gym.set_actor_root_state_tensor_indexed(sim, gymtorch.unwrap_tensor(_initial_root_states),
                                                     gymtorch.unwrap_tensor(env_ids_int32), len(env_ids_int32))
...
running_mean_std = RunningMeanStd(torch.tensor(69)).to(device)
...
running_mean_std.load_state_dict(weights)
step_count= 0
...
while not gym.query_viewer_has_closed(viewer):
    ...
    obs = compute_digit_observations(digit_root_states.unsqueeze(0), dof_states.view(digit_num_dofs,2)[:digit_num_dofs,0].unsqueeze(0), 
                                     dof_states.view(digit_num_dofs,2)[:digit_num_dofs,1].unsqueeze(0),
    ...
    #     break
    # break
    dof_pos[:,0] = action*pd_action_scale+ pd_action_offset
    # print(actio)
    gym.set_dof_position_target_tensor(sim, gymtorch.unwrap_tensor(dof_pos))
    gym.simulate(sim)

    if digit_root_states[2] < 0.8 or digit_root_states[2] > 1.5:
        # digit_dof_pos[:] =_initial_dof_pos
        # digit_dof_vel[:] = 0
        dof_states[:,0] = _initial_dof_pos
        dof_states[:,1] = 0
        gym.set_actor_root_state_tensor_indexed(sim, gymtorch.unwrap_tensor(_initial_root_states),
                                                     gymtorch.unwrap_tensor(env_ids_int32), len(env_ids_int32))
