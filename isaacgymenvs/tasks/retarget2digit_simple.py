...
def get_J(q):
    success = gym.set_actor_dof_states(envs[3], digit_actor_handles[3], q.astype(gymapi.DofState.dtype), gymapi.STATE_POS)
    print(success)
    gym.simulate(sim)
    gym.fetch_results(sim, True)
    gym.refresh_rigid_body_state_tensor(sim)
    ...
    robot_rot = [None]*4
    diff = robot_points[3]-reference_points[3]
    print("error", diff.length())
    gym.step_graphics(sim)
    gym.draw_viewer(viewer, sim, True)
    gym.sync_frame_time(sim)
    print((2*np.array([diff.x, diff.y, diff.z,0,0,0])@j_RF[3].cpu().numpy()).shape)
    return (2*np.array([diff.x, diff.y, diff.z,0,0,0])@j_RF[3].cpu().numpy())[:]

def get_cost_from_q(q):
    success = gym.set_actor_dof_states(envs[3], digit_actor_handles[3], q.astype(gymapi.DofState.dtype), gymapi.STATE_POS)
    print(success)
    gym.simulate(sim)
    gym.fetch_results(sim, True)
...
while not gym.query_viewer_has_closed(viewer):
    ...
    output = scipy.optimize.minimize(get_cost_from_q,digit_dof_positions, method='Newton-CG', jac=get_J, tol=1e-6,options={'maxiter': 10000})
    # print(output)
    digit_dof_positions *=0
    digit_dof_positions += output.x
